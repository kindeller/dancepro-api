...
| 18 | Maintainability | Concert logos are matched using event names | Resolved - pending commit approval | Scheduling events now retain a nullable studio relationship, booking approval records the reviewed studio, existing unambiguous concert-name matches are backfilled, and My Hub resolves logos through the relationship with the event logo as fallback; 229 tests / 1,824 assertions. |
