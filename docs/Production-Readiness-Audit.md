...
| 6 | Medium | Public concert media endpoints have no request throttling | Resolved | Playback lookup, streaming and download routes now have separately configurable per-asset/IP and broader per-IP limits, with regression coverage for all three routes; route listing, 239 tests / 1,864 assertions and Pint pass; `555308d`. |
| 7 | Medium | Replacing private operational files leaves obsolete objects behind | Resolved | Replacing an operational resource, venue map or event programme now removes only its previous object from the private operational disk after the new path is saved; all three flows have regression coverage; 239 tests / 1,873 assertions and Pint pass; `f70f077`. |
| 8 | Medium | Standard HTTP security headers are not applied | Resolved | Global middleware now applies MIME-sniffing, same-origin framing, referrer and browser-feature restrictions, plus one-year HSTS for production HTTPS; CSP is documented as deferred to avoid breaking current inline assets; 241 tests / 1,893 assertions and Pint pass; `34fcb20`. |
| 9 | Medium | The concert-player production bundle exceeds the build warning threshold | Resolved | The 4.3 kB concert entry now lazy-loads the 359 kB HLS.js light build only when native HLS is unavailable; the production build completes without chunk-size warnings and retains MP4 fallback; `c44ab36`. |
| 10 | Medium | JavaScript-heavy workflows lack browser-level regression coverage | Resolved | Playwright now rebuilds production assets and exercises booking-form conditional behavior and Concert playlist selection in desktop and mobile Chromium against Sail's isolated testing database; 4 browser checks, 241 Laravel tests / 1,893 assertions, and both production-only and full npm audits pass with zero vulnerabilities. |
