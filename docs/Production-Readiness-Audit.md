...
| 7 | Medium | Replacing private operational files leaves obsolete objects behind | Resolved - pending commit approval | Replacing an operational resource, venue map or event programme now removes only its previous object from the private operational disk after the new path is saved; all three flows have regression coverage; 239 tests / 1,873 assertions and Pint pass. |
