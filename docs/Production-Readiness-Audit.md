...
| 5 | High | Database backups are not scheduled by the application | Resolved - pending commit approval | The existing verified backup command now runs daily at a configurable local time when enabled, with pruning, overlap prevention and a single-server lock; schedule listing, 238 tests / 1,858 assertions and Pint pass. |
