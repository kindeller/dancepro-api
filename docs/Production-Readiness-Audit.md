...
| 19 | Usability | Email copying has no browser fallback | Resolved - pending commit approval | Studio and competition email copying uses the modern Clipboard API when available and a shared hidden-field copy fallback when access is unavailable or rejected; 230 tests / 1,827 assertions. |
